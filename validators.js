...
import {localeLan} from './App'

export const isRequired = value =>
  value === undefined ||
    value === '' ||
    (Array.isArray(value) && value.length === 0)
    ? i18n.t('errors.required', {locale: localeLan})
    : undefined

export const isEmail = value =>
  value && !validator.isEmail(value)
    ? i18n.t('errors.email', {locale: localeLan})
    : undefined

export const isvalidPassword = value =>
  value && !/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])[0-9a-zA-Z]{6,}$/.test(value)
    ? i18n.t('errors.clientPassword', {locale: localeLan})
    : undefined

export const isconfirmPassword = (value, allValues, props, name) => {
  return value && value === allValues?.masterPassword
    ? undefined
    : i18n.t('errors.confirmPassword', {locale: localeLan})
}
...
export const isPercentage = value =>
  value &&
    ...
    ? i18n.t('errors.percentage', {locale: localeLan})
    : undefined

export const isCountZero = value =>
  value === 0 ? i18n.t('errors.moreThanZero', {locale: localeLan}) : undefined


export const isTwoBigWords = val => {
  if (!val || val?.length !== 2 || !/^[A-Z]*$/.test(val)) {
    return i18n.t('errors.requireTwoUppercaseLetters', {locale: localeLan})
  }
  else
    return undefined
}
...
export const isPositiveInteger = value => {
  return (value >= 0) ? undefined : i18n.t('errors.moreThanZero', {locale: localeLan})
}
