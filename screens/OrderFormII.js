...
class OrderFormII extends React.Component {
        ...
        response.json().then(product => {
          if (
            ...
            dispatchFetchRequestWithOption(
              ...
              response => {
                successMessage(
                  this.context.t("addItemSuccess", {product: product.name})
                );
  ...
  toggleOrderLineItem = lineItemId => {
    const lineItem = this.state?.orderLineItems?.hasOwnProperty(lineItemId)
      ? this.state.orderLineItems[lineItemId]
      : {
        checked: false,
        value: lineItemId
      };
  ...
  handleDeleteLineItem = (orderId, lineItemId) => {
    dispatchFetchRequest(
      api.order.deleteLineItem(orderId, lineItemId),
      {
        method: "DELETE",
        withCredentials: true,
        credentials: "include",
  ...
  handleMergeOrder = (setid, orderId) => {
    dispatchFetchRequest(
      ...
      handleOrderAction(this.props.order?.orderId, "ENTER_PAYMENT", () =>
  ...
  handleItemOutOfStock = (lineItemId, outOfStock) => {
    Alert.alert(
      `${this.context.t(
        outOfStock
          ? `order.toggleUnmarkOutOfStockMsg`
          : `order.toggleOutOfStockMsg`
      )}`,
      ``,
      [
        {
          ...
          onPress: () => {
            dispatchFetchRequest(
              api.product.toggleOutOfStock(lineItemId),
              {
                method: "POST",
                withCredentials: true,
                credentials: "include",
  ...
  getSplitBillByHeadCount = id => {
    dispatchFetchRequestWithOption(
      ...
      response => {
        response.json().then(data => {
          let isPaid =
            data?.splitAmounts?.filter(item => item?.paid)?.length > 0;
        ...
        return (
          <ThemeContainer>
            <View style={{...styles.fullWidthScreen, marginBottom: 0}}>
              ...
              <SockJsClient
                url={`${apiRoot}/ws`}
                topics={[`/dest/order/${order?.orderId}`]}
                onMessage={data => {
                ...
                onConnect={() => {
                  this.orderFormRef.sendMessage(
                    `/async/order/${order?.orderId}`
                  );
              ...
              <View style={{flexDirection: "row", flex: 1}}>
                ...
                <View
                  style={[
                    themeStyle,
                    styles.orderItemSideBar,
                    {
                      borderColor: customMainThemeColor,
                      borderTopWidth: 1,
                      paddingTop: 5
                    },
                    styles?.customBorderAndBackgroundColor(this.context)
                  ]}
                >
                  <ScrollView style={{flex: 1}}>
                    ...
                    >
                      <TouchableOpacity
                        style={[
                          this.state.selectedLabel === "pinned"
                            ? styles?.selectedLabel(customMainThemeColor)
                            : null,
                          {flex: 1}
                        ]}
                        onPress={() => {
                          this.setState({selectedLabel: "pinned"});
                        }}
                      >
                        {this.PanelHeader(
                          t("pinned"),
                    ...
                    {labels.map(lbl => (
                      ...
                      >
                        <TouchableOpacity
                          style={[
                            this.state.selectedLabel === lbl.label
                              ? styles?.selectedLabel(customMainThemeColor)
                              : null,
                            {flex: 1}
                          ]}
                          onPress={() => {
                            this.setState({selectedLabel: lbl.label});
                          }}
                        >
                          {this.PanelHeader(
                            lbl.label,
                    ...
                    >
                      <TouchableOpacity
                        style={[
                          this.state.selectedLabel === "ungrouped"
                            ? styles?.selectedLabel(customMainThemeColor)
                            : null,
                          {flex: 1}
                        ]}
                        onPress={() => {
                          this.setState({selectedLabel: "ungrouped"});
                        }}
                      >
                        {this.PanelHeader(
                          t("product.ungrouped"),
                ...
                >
                            ...
                            return (
                              <TouchableOpacity
                                style={[
                                  ...
                                  prd?.outOfStock && {backgroundColor: "gray"}
                                ]}
                                onPress={() =>
                                  prd?.outOfStock
                                    ? this.handleItemOutOfStock(
                                      prd.id,
                                      prd?.outOfStock
                                    )
                  ...
                  <View
                    style={{flexDirection: "row", flex: 1, padding: "3%"}}
                  >
                    {order.state === "OPEN" && (
                      <>
                        <View
                        ...
                        >
                          <View style={{flex: 1, flexDirection: "column"}}>
                            <TouchableOpacity
                              ...
                              style={[
                                styles?.flexButtonSecondAction(this.context),
                                {marginBottom: 3}
                              ]}
                            >
                              <Text
                            ...
                            <SecondActionButton
                              onPress={() => {
                                if (
                                  ...
                                  splitParentOrderId === order?.orderId
                                ) {
                                  this.setState({
                                    splitBillModalVisible: true
                                  });
                                } else {
                                  Alert.alert(`${t("splittingCheck")}`, ``, [
                                    {
                                      text: `${this.context.t("action.yes")}`,
                                      onPress: async () => {
                                        await revertSplitOrder(
                                          this.context?.splitParentOrderId,
                                          this.context?.splitOrderId
                                        );
                                        await this.context?.saveSplitOrderId(
                                          null
                                        );
                                        await this.context?.saveSplitParentOrderId(
                                          null
                                        );
                              ...
                              onPress={() =>
                                order.lineItems.length === 0
                                  ...
                                  : !!this.props.navigation.state.params
                                    ?.orderSetData &&
                                    this.props.navigation.state.params
                                      ?.orderSetData?.status !== "MERGED"
                                    ? Alert.alert(
                                      `${this.context.t(
                                        "order.mergeOrderTitle"
                                      )}`,
                                      `${this.context.t(
                                        "order.mergeOrderMsg"
                                      )}`,
                          ...
                          <View style={{flex: 1, marginLeft: 5}}>
                            <TouchableOpacity
                              onPress={() =>
                                this.handleComplete(
                                  this.props.navigation.state.params
                                    ?.orderSetData?.mainOrderId ?? order.orderId
                                )
                              }
                              style={styles?.flexButtonSecondAction(
                                this.context
                                  ...
                                  <View style={{flex: 1, borderRadius: 10}}>
                                    ...
                                    >
                                      <StyledText style={{width: 40}}>
                                        {item.price === 0
                                          ? t("order.cancelFreeLineitem")
                                          : t("order.freeLineitem")}
                                      </StyledText>
                                    </TouchableOpacity>
                                  </View>
                                  <View
                                  ...
                                  >
                                    ...
                                    >
                                      ...
                                      {!!item?.appliedOfferInfo && (
                                        ...
                                        >{` ${item?.appliedOfferInfo?.offerName}(${item?.appliedOfferInfo?.overrideDiscount})`}</StyledText>
                                      )}
                                    </View>
                                    <View
                                      style={{
                                      ...
                                      <View style={{marginRight: 5}}>
                                        {item?.state === "OPEN" && (
                                          <StyledText
                                            style={[
                                              {
                                                backgroundColor: `${Colors.orderBgWhite}`,
                                                color: `${Colors.orderOpen}`,
                                                paddingHorizontal: 2,
                                                fontWeight: "bold"
                                              },
                                              !!this.state?.choosenItem?.[
                                        ...
                                        {[
                                          ...
                                          "ALREADY_IN_PROCESS"
                                            ...
                                            <StyledText
                                              style={[
                                                {
                                                  backgroundColor: `${Colors.orderBgGray}`,
                                                  color: `${Colors.orderInProcess}`,
                                                  paddingHorizontal: 2,
                                                  fontWeight: "bold"
                                                },
                                                !!this.state?.choosenItem?.[
          ...
          <ThemeContainer>
                          ...
                          <ListItem
                            ...
                            title={
                              <View style={[styles.tableRowContainer]}>
                                <View
                                  style={[styles.tableCellView, styles.flex(1)]}
                                >
                            ...
                            bottomDivider
                            containerStyle={[
                              styles.dynamicVerticalPadding(10),
            ...
            <View style={[styles?.shoppingBar(customMainThemeColor)]}>
              ...
              <View style={[styles.tableCellView, styles.quarter_width]}>
                ...
                >
                  &nbsp;
                  {order.demographicData != null
                    ? order.demographicData.customerCount
                    : 0}
                </Text>
              </View>
              ...
              >
                ...
                >
                  <View style={{flexDirection: "row", alignItems: "center"}}>
                    ...
                    />
                    <View
                      style={styles?.itemCountContainer(customMainThemeColor)}
                    >
